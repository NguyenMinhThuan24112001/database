...
select count(sex) from customer1 c4 where c4.country = 'VietNam' and c4.sex = false ;

select c4.country from customer1 c4 where c4.sex=false group by c4.country  ;
select (select distinct c1.country from customer1 c1) ,count(sex) from customer1 c2 where c2.sex =false and 10<=(select count(sex) from customer1 c1 where c1.sex =false);